...
is consistent with a square-root error term $X^{-1/2} \log X$, which
matches the function-field analogues established unconditionally by
Keating--Rudnick and Sawin--Shusterman. The results confirm
Hardy--Littlewood in arithmetic progressions quantitatively within
the tested regime, and show that the $\dr$-mod-$9$ classification is
a sensitive empirical probe that does \emph{not} reveal deviations
from the Hardy--Littlewood prediction.
\end{abstract}
...
\subsection{On the magnitude of the empirical error}

The observed deviation $|\rho(g; X) - 1| \le 0.0015$ at $X = 5 \cdot 10^7$
is consistent with an error term of size $X^{-1/2} \log X \approx 0.0024$.
This square-root behaviour is the natural prediction of the Cram\'er
random model of the primes, although that model is known to fail in
some regimes \cite{Mai85}.

For the twin primes conjecture, the same scaling has been verified
empirically at much larger scales: at $X = 10^{18}$, the discrepancy
between the empirical count $\pi_2(10^{18}) = 808{,}675{,}888{,}577{,}436$
and the Hardy--Littlewood prediction $\approx 808{,}675{,}901{,}493{,}606$
is of order $\sqrt{N}/2 \approx 1.3 \cdot 10^7$, consistent with our
observation at smaller scale.
...
classes modulo a squarefree $Q \in \mathbb{F}_q[T]$ with $q \to \infty$,
the variance
\[
  G(n; Q) = \sum_{\substack{A \bmod Q \\ \gcd(A,Q)=1}}
  \Bigl| \Psi(n; Q, A) - \frac{q^n}{\Phi(Q)} \Bigr|^2
  \;\sim\; q^n (\deg Q - 1),
\]
which is the function-field analogue of Hooley's conjecture
$G(X, Q) \sim X \log Q$ on the variance of primes in arithmetic
progressions over $\mathbb{Z}$. More directly relevant to the
present empirical setup, Sawin--Shusterman \cite[Theorem 1.1]{SS22}
established the quantitative twin prime conjecture over
$\mathbb{F}_q[T]$ with a power saving: for an odd prime $p$, a power
$q$ of $p$ with $q > 685090\, p^2$, and any nonzero $h \in \mathbb{F}_q[T]$,
\[
  \#\{f \in \mathbb{F}_q[T] : |f| = X,\ f \text{ and } f+h \text{ are prime}\}
  \;\sim\; \mathfrak{S}_q(h) \, \frac{X}{\log_q^2(X)}
\]
...
the Hardy--Littlewood asymptotic for prime pairs over $\mathbb{Z}$
is the natural conjectural target, in line with the present
empirical observation. No analogous explicit secondary-asymptotic
formula $E(x; g, q, a)$ over $\mathbb{Z}$ appears in the published
literature \cite{MO_question}.
...
comments on MathOverflow \cite{MO_question}, in particular for
pointing to the empirical benchmark for twin primes at $X = 10^{18}$
and to the function-field analogues established in \cite{SS22},
which provide the rigorous context for the empirical error term
observed here.
